...
class _DietPageState extends ConsumerState<DietPage> with RouteAware {
  ...
  Widget build(BuildContext context) {
    ...
    return AsyncBuilder(asyncValue: ref.watch(dietsProvider), builder: (diets) {
      final diet = diets.getDietById(ref.read(pathProvider).dietId!);
      return CustomScaffold(
        header: FutureBuilder(future: diet, builder: (BuildContext context, AsyncSnapshot<Diet?> snapshot) {
          if (snapshot.hasData) {
            return Padding(
        ...
        floatingActionButton: AnimatedFAB(
            ...
            onPressed: () async {
              Meal.showAddMealDialog(
                  context, ref, (await diet)!.id, pageController.page!.round(),
                  editMode: editMode);
            }),
      );
    });
